...
| Human (post-Phase 11) | **The human removed the chapter-level `> **Well-Architected pillars.**` line from §1–§9** (§0's is untouched), directly overriding Phase 11's "exactly one 2–4-pillar line per numbered chapter" acceptance criterion — a deliberate, repeated edit (a broader first pass also dropped the whole §9 chapter and §0.5; the agent restored those and flagged the pillar-line removal, and the human redid the pillar-line cut alone on the next pass, leaving §9/§0.5/all nine ADR citations intact). Not reverted a second time — treated as the human resolving this in favor of a shorter document. Logged here since it contradicts `style-guide.md` §3 / `AGENT-PROTOCOL.md` §5.4's explicit rule and the Phase 11 completion report below (which records pillars tagged per chapter as an "as-shipped" fact that is no longer true in the working tree). Phase 12/13 should treat the missing pillar lines as intentional, not a defect to restore, unless the human says otherwise. | Resolved (by human) | Pillar lines removed from §1–§9 deliberately; §0's line and every other criterion (ADR citations, §9 chapter, §0.5, locked tables) kept intact. |
