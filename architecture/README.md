...
> **Trade-off.** Each account pays its own baseline — GuardDuty, Config, VPC endpoints — at roughly
> $25–60/month, plus a Terraform state file. That cost is why the answer is seven, not twenty
> (Appendix B, ADR-004).
...
> **Trade-off.** Aurora costs more per unit of compute than RDS and is less portable off AWS —
> engineering time is the scarcer resource here, and Aurora spends the least of it.
...
> **Day 1 vs. at scale.** In-cluster `kube-prometheus-stack` costs materially less and is adequate for
> a few hundred users; the reason to move is that a monitor running inside the cluster it watches goes
> blind during a cluster-wide incident. Trigger: the first such incident, or a staffed on-call rotation
> (Appendix B, ADR-026).
...
> **Trade-off.** What breaks this curve: unbounded retention, a chatty integration crossing a NAT
> Gateway on every call, or cross-AZ transfer from a workload ignoring its dependencies' AZ — cheap to
> prevent early, expensive to discover after months of growth.
