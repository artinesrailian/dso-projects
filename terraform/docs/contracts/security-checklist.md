...
| **S-28c** | Developers cannot read secrets **casually**, impersonate a workload identity, or escalate via RBAC | The Role grants no verb on `secrets`, `serviceaccounts`, `roles` or `rolebindings`; asserted by `kubectl auth can-i --as-group` in `verify.sh`. **Stated limitation, not a gap that was missed:** a deliberate user holding `pods: create` can still read any Secret in the namespace by mounting it into a pod they own, and inherits any Pod Identity association bound to that namespace. No RBAC rule prevents either — the boundary is the namespace. See phase-05 §5.3d. |
| **S-28d** | Cross-team credentials never share a namespace | One namespace per team via paired `developer_namespaces` / `governed_namespaces`. A dedicated namespace is a **precondition** for the first Pod Identity association, because that association is namespace-wide and keyed by ServiceAccount name. |
...
| **S-28b** | One developer cannot consume the cluster, bankrupt it, or expose it | Per-namespace `ResourceQuota` covering **compute, storage (`requests.storage`, `persistentvolumeclaims`, per-StorageClass), ephemeral storage, and object counts for every workload kind the Role grants** — not just deployments. Plus a `LimitRange` with `min` as well as `defaultRequest`: `min` is what stops 50 anti-affine replicas at `cpu: 1m` forcing 50 unconsolidatable nodes. `services.loadbalancers` and `services.nodeports` both `0`. |
