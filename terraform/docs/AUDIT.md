...
| S-C5 | Teardown is complete and verified | `scripts/teardown.sh` | 📝 Implemented, not verified | Deletes workloads and Service/Ingress load balancers, polls for the LBs to actually disappear, deletes **NodePools then NodeClaims** while the controller is still alive, then queries EC2 for surviving Karpenter instances two independent ways and **exits 1 rather than continuing** if anything survives. Only then `terraform destroy`. Sweeps launch templates Terraform never knew about, and lists — but does not blindly delete — EBS volumes the CSI driver may have left. Ordering assertion run and passing — Appendix A.9. Never executed against real infrastructure. **REVIEW.md fixes applied:** F-03 — the "prove nothing is left" query previously used `karpenter.sh/managed-by`, a tag Karpenter v1.14.0 no longer sets at all (replaced by `eks:eks-cluster-name` per Karpenter's own v1 migration guide); the query always found nothing, so the check silently proved nothing. Now ANDs the cluster-agnostic `karpenter.sh/nodepool=*` tag with a cluster-scoped tag, checked two independent ways. F-23 — NodePools are now deleted before NodeClaims (cascades via owner references, blocks new launches), so Karpenter cannot re-provision in the gap between "zero instances" and `terraform destroy`. F-24 — the EBS "sweep" is now documented as conditional on the CSI driver's tagging configuration, not claimed unconditionally; a comment in the script and a first-live-apply check confirm which case applies. |
